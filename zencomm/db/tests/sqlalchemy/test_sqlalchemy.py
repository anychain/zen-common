...
class EngineFacadeTestCase(oslo_test.BaseTestCase):
    ...
    @mock.patch('common.db.sqlalchemy.orm.get_maker')
    @mock.patch('common.db.sqlalchemy.engines.create_engine')
...
class PatchStacktraceTest(test_base.DbTestCase):
    ...
    def test_trace(self):
        ...
        # NOTE(viktors): The code in common.db.sqlalchemy.session filters out
        #                lines from modules under common.db, so we should remove
        #                 "common.db/" from file path in traceback.
        import traceback
        orig_extract_stack = traceback.extract_stack

        def extract_stack():
            return [(row[0].replace("common.db/", ""), row[1], row[2], row[3])
