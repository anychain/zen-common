...
class ConnectionQueue(object):
    ...
    def _is_conn_ready(self, conn):
        """
        sometimes the response may not be remove after read at lasttime's connection
        """  
...
class HTTPRequest(object):
    ...
    def __str__(self):
        return (('method:(%s) protocol:(%s) header(%s) host(%s) port(%s) path(%s) '
                 'params(%s) body(%s)') % (self.method, self.protocol, self.headers,
...
class RestClient(object):
    ...
    def __init__(self, host, port, protocol, pool = None, msg_time_out = 10, http_socket_timeout = 10):
        '''
    ...
    def _new_conn(self):
        ...
        if self.secure:
            return httplib.HTTPSConnection(self.host, self.port, timeout = self.http_socket_timeout)
        else:
            return httplib.HTTPConnection(self.host, self.port, timeout = self.http_socket_timeout)
        ...
        while retry_cnt < retry_time:
            ...
            try:
                ...
                if response.status in [200, 201, 204]:
                    ...
                    logging.error("Request failed with status: %d, error: %s " % (response.status, response.read()))
                    conn = self._get_conn()
            except Exception, e:
            ...
            logging.warn("retry request for [%d] time after sleep for [%.2f] secs" % (retry_cnt, next_sleep))
        
        logging.error("send request failed after retry for [%d] times" % (retry_cnt))        
        return None
